...
Native image generation may use the application-owned, loopback-only actor-authorized Provider relay required by Codex App Server for custom providers. The relay may validate the runtime actor, inject the service-owned upstream credential, allowlist Provider routes, and stream the single upstream response; it may not implement image generation, issue a duplicate provider call, synthesize an `imageGeneration` Item, or expose a browser/BFF image-generation endpoint.
...
Marketplace search-language policy is also master data. An official market enum becomes executable only when it intersects an immutable market-profile revision containing query locales, accepted scripts/languages, currency, timezone, sampling bounds, and quality policy. New Harness contracts submit immutable research tasks and immediately return `task_id`; pricing and scope planning are internal backend concerns, not a model-facing free-plan/execute sequence. Durable workers own execution, fenced checkpoints, safe retry and uncertain-result reconciliation; execution may not accept mutable business scope. Discovery must materialize immutable, diversified target-specific step instances rather than one global provider binding. Multilingual relevance uses original and localized variants; valid low-confidence data is held, never mislabeled as irrelevant merely for missing a model threshold.

The model-facing market-option tool must not expose internal quality thresholds, profile identifiers/revisions, or representative-sample maxima. Unless the user explicitly asks for a count, task submission uses the profile default. A quota-driven coverage reduction is asked only after backend validation and must use native `request_user_input`; never render numbered choices in an assistant message or create a second user Turn for the answer.
