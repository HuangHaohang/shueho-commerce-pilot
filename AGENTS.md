...
Marketplace search-language policy is also master data. An official market enum becomes executable only when it intersects an immutable market-profile revision containing query locales, accepted scripts/languages, currency, timezone, sampling bounds, and quality policy. New Harness contracts must use free `plan_marketplace_research` followed by paid `execute_marketplace_research(plan_id)`; execution may not accept mutable business scope. Discovery must materialize immutable, diversified target-specific step instances rather than one global provider binding. Multilingual relevance uses original and localized variants; valid low-confidence data is held, never mislabeled as irrelevant merely for missing a model threshold.

The model-facing market-option tool must not expose internal quality thresholds, profile identifiers/revisions, or representative-sample maxima. Unless the user explicitly asks for a count, planning uses the profile default. A quota-driven coverage reduction is asked only after the free quote and must use native `request_user_input`; never render numbered choices in an assistant message or create a second user Turn for the answer.
