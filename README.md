...
- Provides a three-pane Creative Space that maps each project to one persisted Codex thread. A closed, application-managed method registry adds native specialist Skill Items for listing copy, promotion copy, main images, gallery images, detail pages, shooting scripts, and short-video storyboards while the project Skill keeps Harness ownership of the conversation. Completed Harness Items materialize into a tenant-owned infinite canvas with document, native-image-plus-text-layer, and script-table nodes; application tables persist layout and append-only manual revisions without replacing conversation history or native image artifacts. Assistant replies and canvas nodes retain an exact Item-to-node mapping for bidirectional focus. Product-accurate image work still requires a tenant-owned reference image or attachment; a catalog `image_url` is never fetched as an arbitrary host/network input. Rendered video remains explicitly disabled until a governed application video tool, approval, asynchronous job, tenant artifact, and readback exist. See [Creative Space Workbench](./docs/architecture/creative-space-workbench.md).
...
The Gateway enables Codex Harness's built-in `image_gen` capability and consumes native `imageGeneration` Items; it does not expose `commerce_image.generate` or a direct image-generation HTTP route. Native image bytes and host paths are stripped before browser fan-out, while completed files are stored under the tenant-owned `$CODEX_HOME/generated_images` and served only through ownership-checked artifact reads. Usage remains part of the Harness response stream.
