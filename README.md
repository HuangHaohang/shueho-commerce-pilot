...
- Routes every governed JustOneAPI request through one client with a protected token pool, persistent per-interface round-robin selection, atomic quota reservation/debit, scoped exhaustion and immutable attempt receipts. See [token quota operation](./docs/deployment/justoneapi-tokens.md).
