...
- Supports an optional JustOneAPI-only subscription proxy pool with per-request random rounds, TLS health probes, cooldown/recovery and no direct fallback or post-dispatch retry. See [protected import and deployment](./docs/deployment/justoneapi-proxy.md).
- Routes every governed JustOneAPI request through one client with a protected token pool, random selection among eligible per-interface keys, atomic dispatch accounting, provider-confirmed scoped exhaustion and immutable attempt receipts. See [token quota operation](./docs/deployment/justoneapi-tokens.md).
...
- Tool contract 6 also exposes the complete database-backed provider catalog through `search_data_capabilities`, `get_data_capability`, free `plan_data_request` and governed `execute_data_request(plan_id)`, with distinct pricing/permission/quota availability and validated source-field observations for content, metrics, profiles and AI answers.
