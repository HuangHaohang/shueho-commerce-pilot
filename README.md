...
- Uses the App Server thread queue for running-turn submissions, including native add/list/update/delete operations. “调整方向” keeps the message in that queue, interrupts the superseded Turn, waits for authoritative `turn/completed`, and starts the same queued submission through `thread/queue/start`; no second pending-message store exists.
