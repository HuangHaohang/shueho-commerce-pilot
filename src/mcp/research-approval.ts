...
export function researchApprovalHandler(server:McpServer,upstream:ExternalDataServiceMcpClient,control:ExternalDataControlClient,p:AuthenticatedMcpPrincipal){
 ...
 async function resolve(id:string,signal?:AbortSignal,native=false,seen?:Set<string>){
  ...
  let live=await control.revalidate(p,approval.reservationId);
 ...
 return (id:string,signal?:AbortSignal,native=false,seen?:Set<string>)=>{
  const existing=pending.get(id);if(existing)return existing.then(task=>{if(task.approval?.reservationId)seen?.add(task.approval.reservationId);return task;});
  const work=resolve(id,signal,native,seen).finally(()=>{if(pending.get(id)===work)pending.delete(id);});pending.set(id,work);return work;
 };
}
