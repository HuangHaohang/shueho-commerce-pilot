...
export async function drainResearchSettlement(upstream:ExternalDataServiceMcpClient,control:ExternalDataControlClient){
 const lease=randomUUID();const claim=await upstream.taskOperation('claim_research_settlement',{lease_id:lease,issued_at:new Date().toISOString()});
 ...
 try{
  const intent=job.payload as unknown as Record<string,string>;
  if(intent.kind==='cancel_source')await control.cancelSource(job.principal,intent.source as 'external_mcp'|'codex_harness',intent.callId!);
  else if(intent.kind==='cancel_reservation')await control.cancel(job.principal,intent.reservationId!,'user_denied');
  else await control.settle(job.principal,job.reservation_id,job.payload);
  succeeded=true;
 }
...
export function startResearchSettlementWorker(upstream:ExternalDataServiceMcpClient,control:ExternalDataControlClient){
 let busy=false;const idle=new IdleBackoff();const timer=setInterval(()=>{if(busy||!idle.ready())return;busy=true;void drainResearchSettlement(upstream,control).then(work=>idle.observed(work)).catch(()=>{idle.observed(false);console.error(JSON.stringify({event:'research_settlement_worker_unavailable'}));}).finally(()=>{busy=false;});},2000);
