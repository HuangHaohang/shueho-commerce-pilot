...
export function registerDataCapabilityTools(server: McpServer, principal: AuthenticatedMcpPrincipal,
  ...
  if (principal.scopes.includes("external_data.catalog.read")) {
    server.registerTool("search_data_capabilities", {title:"搜索全部数据能力",description:DATA_CAPABILITY_TOOL_DESCRIPTIONS.search_data_capabilities,
      ...
      try {return success(requireDataPayload(await upstream.searchDataCapabilities({...args,...await authorization()})));} catch(error) {rethrowResearchRecovery(error);return failure(error);} });
    server.registerTool("get_data_capability", {title:"查看数据能力参数",description:DATA_CAPABILITY_TOOL_DESCRIPTIONS.get_data_capability,
      ...
      try {return success(requireDataPayload(await upstream.getDataCapability({...args,...await authorization()})));} catch(error) {rethrowResearchRecovery(error);return failure(error);} });
  }
  ...
  server.registerTool("plan_data_request", {title:"免费规划数据查询",description:DATA_CAPABILITY_TOOL_DESCRIPTIONS.plan_data_request,
    ...
    } catch(error) {rethrowResearchRecovery(error);return failure(error);}
  });
  ...
  async function execute(planId: string) {
    ...
    try {
      ...
      try {result = await upstream.executeDataRequestPlan({plan_id:planId,_commerce_context:scope});}
        ...
        await control.settle(principal,reservation.reservationId,{state:"unknown",upstreamCode:null,upstreamMessage:"Data execution response is uncertain.",resultBytes:null,responsePayload:null}).catch(error=>{rethrowResearchRecovery(error);if(error instanceof SettlementNotPersistedError)throw error;});
      ...
      catch(error) {rethrowResearchRecovery(error);if(error instanceof SettlementNotPersistedError)throw error;settlementPending=true;}
