...
try {
  ...
  const rows = await database.query(`SELECT token.token_id,token.state AS token_state,token.updated_at AS token_updated_at,quota.api_path,
    quota.remaining_calls AS legacy_remaining_calls,quota.reserved_calls,quota.used_calls,quota.inflight_calls,quota.state,
  ...
  console.log(JSON.stringify({ selectionPolicy: "provider_feedback", localRemainingEnforced: false, summary: await new PostgresJustOneApiTokenStore(database).status(ids), tokens, quotas: rows.rows }, null, 2));
} catch {
