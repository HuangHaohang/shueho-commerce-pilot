...
import {readResearchRecords,readTaskRecords} from "./research-records.js";
...
export function createExternalDataMcpServer(pipeline = new ExternalDataPipeline()): McpServer {
  ...
  server.registerTool('claim_research_settlement',{inputSchema:{lease_id:z.string().uuid()}},async a=>toolSuccess(await claimSettlement(a.lease_id)));
  ...
  server.registerTool('read_research_records',{inputSchema:{snapshot_id:z.string().uuid().optional(),task_id:z.string().uuid().optional(),research_request_id:z.string().uuid().optional(),offset:z.number().int().min(0).max(10000).default(0),limit:z.number().int().min(1).max(100).default(50),_commerce_context:taskScope}},async a=>{if(!!a.task_id===!!a.research_request_id)throw new Error("Exactly one task_id or research_request_id is required");return toolSuccess(a.task_id?await readTaskRecords(taskOwner(a._commerce_context),a.task_id,a.offset,a.limit,a.snapshot_id):await readResearchRecords(taskOwner(a._commerce_context),a.research_request_id!,a.offset,a.limit));});
