...
export function createExternalDataMcpServer(pipeline = new ExternalDataPipeline()): McpServer {
  ...
  server.registerTool('manage_research_task',{inputSchema:{action:z.enum(['list','cancel','resume']),task_id:z.string().uuid().optional(),cursor:z.string().uuid().optional(),limit:z.number().int().min(1).max(50).optional(),_commerce_context:taskScope}},async a=>toolSuccess(await manageResearchTask(taskOwner(a._commerce_context),a as JsonObject)));
