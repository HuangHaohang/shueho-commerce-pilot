...
import {submitResearchTask,readResearchTask,claimResearchTask,updateResearchTask,manageResearchTask,researchQueueHealth} from "./research-tasks.js";
...
export function createExternalDataMcpServer(pipeline = new ExternalDataPipeline()): McpServer {
  ...
  const taskScope=z.object({tenant_id:z.string().uuid(),workspace_id:z.string().uuid(),user_id:z.string().min(1),root_thread_id:z.string().nullable().optional()});
  const taskOwner=(s:z.infer<typeof taskScope>)=>({tenantId:s.tenant_id,workspaceId:s.workspace_id,userId:s.user_id,rootThreadId:s.root_thread_id});
  ...
  server.registerTool('enqueue_research_settlement',{inputSchema:{task_id:z.string().uuid(),reservation_id:z.string().uuid(),payload:z.object({state:z.enum(['succeeded','business_failed','unknown']),upstreamCode:z.number().int().nullable(),upstreamMessage:z.string().max(500).nullable(),resultBytes:z.number().int().nullable(),responsePayload:z.record(z.unknown()).nullable()}),_commerce_context:taskScope}},async a=>toolSuccess(await enqueueSettlement(taskOwner(a._commerce_context),a.task_id,a.reservation_id,a.payload)));
  server.registerTool('claim_research_settlement',{inputSchema:{lease_id:z.string().uuid(),issued_at:z.string().datetime()}},async a=>toolSuccess(await claimSettlement(a.lease_id,a.issued_at)));
  server.registerTool('finish_research_settlement',{inputSchema:{reservation_id:z.string().uuid(),lease_id:z.string().uuid(),succeeded:z.boolean(),error_code:z.string().max(100).nullable(),_commerce_context:taskScope}},async a=>toolSuccess(await finishSettlement(taskOwner(a._commerce_context),a.reservation_id,a.lease_id,a.succeeded,a.error_code)));
  server.registerTool('read_research_records',{inputSchema:{cursor:z.string().max(512).optional(),snapshot_id:z.string().uuid().optional(),task_id:z.string().uuid().optional(),research_request_id:z.string().uuid().optional(),offset:z.number().int().min(0).max(2147483647).default(0),limit:z.number().int().min(1).max(100).default(50),_commerce_context:taskScope}},async a=>toolSuccess(await readRecordPage(taskOwner(a._commerce_context),a)));
  server.registerTool('manage_research_task',{inputSchema:{action:z.enum(['list','cancel','resume']),approval_reservation_id:z.string().uuid().optional(),task_id:z.string().uuid().optional(),cursor:z.string().max(512).optional(),limit:z.number().int().min(1).max(50).optional(),_commerce_context:taskScope}},async a=>toolSuccess(await manageResearchTask(taskOwner(a._commerce_context),a as JsonObject)));
  server.registerTool('research_queue_health',{inputSchema:{}},async()=>toolSuccess({success:true,...await researchQueueHealth()}));
  ...
  server.registerTool('claim_research_task',{inputSchema:{lease_id:z.string().uuid(),issued_at:z.string().datetime()}},async a=>toolSuccess(await claimResearchTask(a.lease_id,a.issued_at)));
  server.registerTool('update_research_task',{inputSchema:{task_id:z.string().uuid(),lease_id:z.string().uuid(),action:z.enum(['heartbeat','finish','retry','wait','read_operation','begin_operation','complete_operation']),state:z.enum(['completed','partial','failed','reconciliation_required','waiting_approval','cancelled']).optional(),error_code:z.string().regex(/^[A-Z0-9_]{1,100}$/).optional(),approval:z.record(z.unknown()).optional(),operation_key:z.string().max(200).optional(),operation_context:z.object({source:z.enum(['external_mcp','codex_harness']),callId:z.string().regex(/^[A-Za-z0-9_-]{8,128}$/)}).optional(),input_hash:z.string().optional(),operation_name:z.string().optional(),result:z.unknown().optional(),_commerce_context:taskScope}},async a=>toolSuccess(await updateResearchTask(taskOwner(a._commerce_context),a as JsonObject)));
  ...
  server.registerTool("execute_data_request_plan", {
    ...
    catch(error) {if(error instanceof DataCapabilityError || error instanceof JustOneApiError && !error.uncertain)return toolError(error.code,safeToolMessage(error),{providerDispatched:false,dispatchPhase:"not_dispatched",research_request_id:args.plan_id});return toolError("DATA_EXECUTION_UNCERTAIN",safeToolMessage(error),{research_request_id:args.plan_id,recovery_tool:"get_research_result"});} });
