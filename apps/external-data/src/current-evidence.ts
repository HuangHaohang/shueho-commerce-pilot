...
export async function currentPromotedEvidence(
  ...
  return withScope(scope, async (client) => {
    const result = await client.query<{ result_key: string; model_metadata: JsonObject }>(`
      ...
      SELECT observation.result_key,decision.model_metadata FROM observations observation
    ...
    `, [ids]);
    return new Map(result.rows.map((row) => [row.result_key, row.model_metadata]));
  });
}
