...
describe("owned image inventory without conversation replay", () => {
  ...
  it("only projects owned safe artifacts, never paths or base64", async () => {
    const artifact = { filename: "1789000000000-aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee.png", threadId, turnId: "turn-owned", model: "image-model", quality: "high", sourceFilenames: [], path: "/private/image", base64: "private" };
    ...
    expect(body.images[0].quality).toBe("high"); expect(body.images[0]).not.toHaveProperty("path"); expect(body.images[0]).not.toHaveProperty("base64");
