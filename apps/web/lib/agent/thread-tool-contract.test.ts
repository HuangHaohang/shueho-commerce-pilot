...
describe("agent dynamic tool contract", () => {
  it("accepts only the product-catalog-aware contract", () => {
    expect(CURRENT_AGENT_TOOL_CONTRACT_VERSION).toBe(9);
    expect(isSupportedAgentToolContractVersion(9)).toBe(true);
    expect(isSupportedAgentToolContractVersion(8)).toBe(false);
  });
});
