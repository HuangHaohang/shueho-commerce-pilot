...
let monthlyLimit:number|null=100,calls=1;
let token=true,permission=true,price=200000,spent=200000,policyMode='policy';
beforeEach(()=>{
 monthlyLimit=100;calls=1;token=true;permission=true;price=200000;spent=200000;policyMode='policy';
 mock.query.mockImplementation(async(sql:string)=>{
  ...
  if(sql.includes('FROM commerce_external_data_policy'))return {rows:[{status:'enabled',approval_mode:policyMode,allowed_platforms:['fixture'],allowed_endpoint_ids:[],monthly_call_limit:monthlyLimit,monthly_spend_limit_micros:500000000,per_call_auto_approval_micros:null}]};
  ...
  if(sql.includes('unresolved_amounts'))return {rows:[{calls_used:String(calls),spend_used_micros:String(spent),unresolved_amounts:'0'}]};
...
describe('live dispatch revalidation',()=>{
 ...
 it('null call limit permits over 100 calls while retaining the monthly money limit',async()=>{monthlyLimit=null;calls=101;expect((await revalidateExternalDataCall(scope,'reservation')).state).toBe('dispatched');spent=500000001;await expect(revalidateExternalDataCall(scope,'reservation')).rejects.toMatchObject({code:'EXTERNAL_DATA_BUDGET_EXCEEDED'});monthlyLimit=100;spent=0;await expect(revalidateExternalDataCall(scope,'reservation')).rejects.toMatchObject({code:'EXTERNAL_DATA_BUDGET_EXCEEDED'});});
