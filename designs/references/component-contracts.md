...
- 长对话在桌面端显示对话区左侧时间线缩略滚动条。刻度必须按真实 DOM 锚点与 `scrollHeight` 计算；默认状态下所有弱化刻度左侧对齐且等长。指针进入轨道后自动选取最近刻度，中心刻度变深并最长，前后相邻刻度按距离对称递减，以 `150ms` 左右的宽度过渡形成金字塔轮廓；离开轨道后全部收回为等长。短对话或无溢出时隐藏，移动端不显示，并在 `prefers-reduced-motion` 下取消宽度动画。
