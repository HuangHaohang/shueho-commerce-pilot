...
- “我的创作”以创作者个人驾驶舱组织任务事实，必须回答当前制作环节、今日重点、下一步动作、协作动态和最近产出；不得复刻任务系统或使用统计卡片墙。制作轨道可以筛选“待我推进”，今日重点必须是页面唯一视觉中心。
- “我的创作”的任务状态、动态与产出映射必须位于独立 adapter/selector；组件仅消费创作者视角模型，并预留策划、拍摄、剪辑、审核角色的排序差异。正式 BFF 未接入时可使用集中 Mock，但必须明确其非实时数据。
