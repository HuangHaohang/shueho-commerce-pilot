...
There is no direct Gateway or public BFF image-generation endpoint. Image creation uses the native Harness `image_gen` tool inside an admitted Turn. Gateway persists completed `imageGeneration` Items, strips image bytes and host paths from browser events/history, and keeps artifact reads ownership-checked and non-cacheable.
