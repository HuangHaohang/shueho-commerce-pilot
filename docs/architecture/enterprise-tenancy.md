...
No direct browser/BFF or Gateway image-generation route is exposed. Images can be generated only inside an already admitted Harness Turn, through the namespace extension or the application-owned Harness projection of Provider-hosted Responses output. Gateway persists only the completed `imageGeneration` Item, strips raw bytes and host paths before browser delivery, and the authenticated BFF re-checks the artifact's owning thread before proxying bytes. New usage is attributed to `codex_harness`; `commerce_image_tool` remains only as a historical enum value.
