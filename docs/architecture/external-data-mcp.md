...
Public responses use stateless Streamable HTTP SSE with native SDK keepalive comments every ten seconds. These transport frames carry no model history or paid-call progress and do not authorize retries or recovery. Reverse proxies must disable buffering, caching and upstream retries. See the [public MCP deployment](../../deploy/production-mcp/README.md) for deployment artifacts and the [production host](../deployment/production-host.md) for the deployment target.

Marketplace execution returns a pending receipt after a 15-second foreground wait when the admitted operation is still running; the same server operation continues, without another provider dispatch or agent turn. The receipt carries the existing plan UUID as `research_request_id` and instructs clients to use `get_research_result`, which accepts plan, execution or research IDs. Polls read current scoped state and completed evidence without settling running workflows. Clients must not call execute again after a pending receipt. Runtime restarts do not replay an interrupted operation; persisted unknown states require reconciliation. Uncertain errors carry structured recovery identifiers, and plaintext SDK failures retain their original cause rather than becoming JSON parser errors.
