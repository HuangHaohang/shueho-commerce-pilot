...
Product enrichment revision `commerce-relevance-v3` requires semantic-model support even when the target word appears verbatim in a product title. Valid low-confidence products remain held. For generic cookware requests, explicitly electric or medicinal product subtypes are held unless the target itself requests that subtype; they are not mislabeled as corrupt or deleted from source storage. Reprocessing appends a new decision revision, keeps prior evidence for provenance, rebuilds derived metrics from the new job, and lets both vector and index search return only observations authorized by the latest completed SQL enrichment job.
