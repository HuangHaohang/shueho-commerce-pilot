...
`provider_market_option` is rebuilt from the imported OpenAPI `site`, `country`, `region` and `market` enums and linked to the same immutable catalog receipt. Localized display labels are generated during import from the runtime CLDR/ICU region data and persisted as master data. Harness first reads active keyword-product workflows through the free `list_marketplace_research_platforms` business tool, then reads site choices through `get_marketplace_options`; Skill text, Gateway code and frontend components contain no marketplace or site option list. A platform without an active complete workflow is explicitly unavailable rather than being represented as a platform with no site dimension.
...
For JD, the business workflow is keyword search -> quality-checked item identifier -> product detail -> price. Similar platform workflows use ASIN, product id, item id and shop id only as private step bindings. `research_workflow_binding_evidence` stores the selected source row, JSON pointer, exact source field and identifier hash. A downstream paid call is never dispatched when the discovery result did not pass quality/relevance promotion or cannot prove every required identifier.
...
For commerce-product responses, only product records with a bounded provider identifier become model candidates. Scalar presentation arrays such as `price_texts` remain fully retained in the source layer but do not consume model context or inflate rejection counts. Cross-market workflows preserve the user's original concept and use an Agent-supplied local-market search term as an explicit enrichment/query input; the multilingual reranker remains the relevance authority rather than a hard-coded translation table.

`ai_decision_review` supports SQL-only human corrections without mutating model history. `quality_evaluation_case`, `quality_evaluation_run` and `quality_evaluation_result` maintain a repeatable Chinese-commerce regression set across products, brands, properties and social content; `npm run external-data:evaluate` records model scores and fails below 90% accuracy.
