...
The unified provider client applies SQL-backed global/per-endpoint admission before reserving an eligible token/interface pair. Explicit documented `301`/`302` rejections can be retried and confirmed `100`/`303`/`601`/`602` refusals can fail over to another eligible key under the same immutable call, at most three attempts by default, with complete attempt archives, jittered backoff and shared endpoint cooldown honoring `Retry-After`. Each actual attempt increments usage/in-flight counters; historical remaining-call values are neither decremented nor admission gates. Provider feedback owns key/interface eligibility, independently of enterprise budgets. Uncertain transport/5xx outcomes, successes and permanent failures are never replayed. Expiring capacity leases and durable polling progress keep failures visible without weakening raw-call fencing; see [deployment controls](../deployment/justoneapi-tokens.md#bounded-retries-and-shared-admission).
...
## Semantic relevance correction (assessment v5, Harness tool contract 12)

Commerce Pilot remains built on the Codex open-source Harness. The Harness may pass explicit user qualifiers as optional semantic_scope: { include: string[], exclude: string[] } on social and marketplace submissions. They are pinned in immutable plan/request JSON. Scope comes from user requirements, never a new backend intent model or keyword dictionary. Existing requests use existing structured target/category fields. originalRequest remains an audit input, not a semantic query. Existing JSON and inclusion/exclusion arrays hold these optional fields; no table migration or mutation of historical receipts is required.

Equivalent original/localized/script queries run independently. Admission requires both existing configured score gates on the same variant. Exclusions are separate positive relevance checks: an excluded-scope match holds the record, never deletes it. Metrics and dates are independently checked. The handcrafted 0.25/0.25/0.5 score and 0.08 lexical admission allowance are removed; relevance scores are Reranker output, not calibrated probabilities or commerce rankings. Every inference forwards its own instruction through Sentence Transformers predict(prompt=...), including after warmup.

New jobs append commerce-relevance-v5; old raw/source/decision rows remain. Coverage separates source-field availability, actual exclusion counts, requested/accepted counts and bounded pagination metadata. Empty delivery is no_samples, not proof of upstream missing fields. Content limits use persisted top_n. A completed invocation with more upstream pages reports collectionComplete=false. No new paid pagination algorithm or automatic historical recollection is included.

The [hardcoding and Harness audit](research-hardcoding-audit.md) records remaining endpoint, market, collection and runtime-boundary work. evaluate.ts runs real production-path regression in addition to the existing DB evaluation.

## Governed research execution (task execution version 3)

Migration 044 adds immutable research_policy_import_receipt rows, joined to the official catalog receipt. The operator import validates endpoint schemas, fixed enum values, pagination keys, IANA zones and source documentation before activation. Social capability selection reads one exact profile, never compares schema size or hardcodes supplier sort meanings. Date windows use that profile's timezone. Parameter defaults come exclusively from imported request schemas; unknown currency remains unknown.

New tasks follow the profile's page or cursor protocol through the existing durable business worker. A private continuation read validates the owned completed source call against the pinned profile and returns the next normalized parameters and an opaque source identity signature. Each page uses an independent existing reservation/approval/dispatch/archive/settlement path. Collection stops at requested qualified coverage, source end, repeated page/cursor, an unidentifiable continuation, or governance/cancellation. There is no fixed derived page budget or automatic replay. Version 1/2 tasks retain single-call social behavior. Outcomes include per-page coverage and stop reason; sample rankings do not claim platform-wide completeness.

Sampling parameters retain their established values in the reviewed policy receipt and are pinned into new marketplace plans. Old plans use the earliest immutable baseline receipt. Retrieval capacities/RRF settings come from the same policy; candidate capacities cannot silently fall below the caller's requested result count. Product, brand and property compact reads use persisted request top_n.

Title generation and the internal web-search model use bounded native Codex App Server ephemeral threads/turns, not direct model HTTP calls. The wrapper refuses client tool/permission requests, retains native completion/error/usage semantics, and never retries an uncertain turn. Titles continue using COMMERCE_TITLE_MODEL (gpt-5.3-codex-spark). Hosted-search provider compatibility uses the pinned official models.json through native model_catalog_json startup configuration; only the Responses Lite wire-mode flag changes in the runtime copy, never the model id. Real search/open Items precede any returned citations.
