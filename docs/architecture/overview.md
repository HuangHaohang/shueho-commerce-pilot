...
| Complete data capability access | Database-generated capability discovery and schemas, immutable single-query plans, governed execution and separately labelled validated source observations |
| Provider availability | Shared PostgreSQL admission, bounded retries for explicit non-billable rejections, immutable attempt receipts and deadline-aware polling; uncertain calls are never replayed |
...
The research recovery patch keeps all agent lifecycle, native questions and permission requests in Codex Harness. The deterministic MCP approval adapter handles only application-owned paid-read approvals and binds continuation to a reservation ID. Versioned record projections preserve original snapshots; financial checkpoint recovery reads live governance receipts and never infers supplier success.
