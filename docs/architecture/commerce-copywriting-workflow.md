...
App Server treats question answers as tool output and does not return them as `thread.turns[].items` during `thread/read`. Commerce Pilot therefore formats one server-authoritative, secret-redacted `我的选择` summary. The Gateway persists it into Harness history with `thread/inject_items`; the authenticated BFF also stores the same summary in the RLS-protected `commerce_agent_user_input_answer` display index. The browser appends it optimistically as a user bubble, and thread reads merge the display index before the final assistant answer so refresh preserves the conversation order. Raw question payloads and secret answers are not stored in the display index.
