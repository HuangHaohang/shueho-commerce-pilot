...
Thread-history readback restores those selections on the authoritative default user message for each Turn. The BFF performs one batch query for the page, filters by the authenticated tenant, workspace, user, thread and requested Turn ids, and joins through the `product_revision_id` fixed in each context item. It projects only bounded Product summaries; attributes, raw source rows, connector configuration and credentials are excluded. A live `product_catalog.read` check is required on every history request. If it is denied or unavailable, authoritative default messages return an empty Product list so the client can clear any stale optimistic selection without receiving catalog data.
