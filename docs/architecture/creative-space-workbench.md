...
- **Edit** turns the selected image into a non-destructive layer document. The native image is a locked base layer; text, rectangles, ellipses, freehand drawings, and reusable base-image copies are independent ordered layers with position, size, rotation, opacity, visibility and lock state.
