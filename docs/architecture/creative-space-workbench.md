...
- **Edit** turns the selected image into a non-destructive Fabric.js design document. The native image is a locked base layer; text, rectangles, ellipses, freehand drawings, uploaded tenant Logo/images, and reusable base-image copies are independent ordered layers with position, size, rotation, opacity, visibility and lock state. Fabric supplies the MIT-licensed browser object engine only; the workbench shell, toolbar, layer panel, properties and storage remain application-owned UI.

Direct layer operations happen on the image itself: users select, drag and resize objects on the artboard, then use the right-side layer and property panels for ordering, rename, rotation, color, typography, opacity, visibility and locking. Saving appends one `user` revision to the existing canvas node and never rewrites native image bytes or calls an image model. The legacy `textLayers` projection remains synchronized for older canvas readers, while `editorLayers` is the authoritative direct-edit scene graph. Image layers may reference only the immutable owned base artifact; arbitrary browser URLs are rejected.

The editor also provides bounded undo/redo, generic square/portrait/landscape artboards, canvas background color, image-layer crop windows, brightness/contrast/saturation/blur filters and PNG/JPG export. Generic aspect ratios are UI layout choices rather than marketplace specifications. Future platform-specific sizes and safe zones must come from active provider/platform master data, never frontend constants.

Logo and image uploads use `POST /api/agent/threads/{threadId}/canvas/assets`. BFF and Gateway re-check `agent.run`, thread ownership, rate/size/MIME limits and binary signatures. Gateway stores successful PNG/JPEG/WebP files as `purpose=canvas_asset`, with at most 100 assets and 100 MB per thread; these artifacts are tenant/workspace/user/thread-bound, remain separate from Harness Turn attachments, cannot become `localImage` inputs through the ordinary attachment contract, and are deleted with the thread. A saved `canvas_asset` image layer contains only its UUID and bounded display name; every revision write revalidates the asset through Gateway metadata before persistence.

AI-assisted pixel changes are an explicit optional section below the layer editor, not the editing model. They start a new Harness Turn in the same creative-project thread. The browser submits only bounded generated-image filenames through `imageEditSourceFilenames`; the authenticated BFF verifies that the persisted Recipe is `creative_project`, and Gateway reloads each artifact from the application-owned `GeneratedImageStore`, requires exact thread ownership, and converts it to a native `localImage` input. Browser URLs, host paths, base64, Provider credentials, masks, raw App Server inputs, and Provider identity are never accepted.
...
Freehand drawing is an editable vector layer in the application scene graph, not a claim of a Provider-native pixel mask. A future true model mask tool may be added only when the selected Harness/Provider contract can preserve mask semantics through the same owned Turn and native Item lifecycle.
